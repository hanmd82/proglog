...
// Write(uint32, uint32) appends the given offset and position to the index.
...
// Close() checks that the memory-mapped file has synced to the persisted file,
// checks that the persisted file has flushed its contents to stable storage, then
// truncates the persisted file to the size of actual data, and closes the file.
